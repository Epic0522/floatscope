...
- Long-text editor for larger drafts, plus right-click "Edit and resend" on user messages.
...
If an agent accepts a message but never starts streaming a reply, FloatScope surfaces a timeout message instead of leaving the capsule in a silent waiting state. For transient network or provider problems, right-click a user bubble, choose **Edit and resend**, adjust the text, and send it again.
...
如果某个 agent 接收了消息但迟迟没有开始输出，FloatScope 会显示超时提示，而不是让浮窗静默等待。遇到临时网络或 provider 问题时，可以右键用户气泡，选择 **重新编辑并发送**，改完后再发送。
