...
It is intentionally quiet: no Live2D, no VRM, no character renderer, and no separate long-term memory database.
...
- Multi-agent configuration through a user-editable JSON file.
...
FloatScope is designed around GUI actions, not typed control commands.
...
Screen watch uses the interval configured in Settings. The app does not continuously capture the screen unless Screen Watch or Screen Replay Cache is enabled.
...
Open Settings from the status bar bubble and configure each agent path before sending messages. Public defaults intentionally leave executable paths blank.
...
You can add more agents from Settings. Unknown adapters can start as `generic-cli` until a dedicated bridge is implemented.
...
The first screen capture may trigger the macOS Screen Recording permission prompt. After granting permission, restart FloatScope if macOS does not immediately allow capture.

### Privacy Model

- No public default contains personal names, private paths, or executable paths.
- FloatScope keeps only lightweight UI state, settings, recent local transcripts, and temporary screenshot files.
- Long-term memory and full conversation history belong to the configured agent applications or CLIs.
- Screen capture is disabled by default except for manual capture.
...
Generated build products live in `.build/` and are not tracked. Local transcripts, screenshots, and runtime agent configs are excluded from git.
...
它的方向是克制、轻量、可长期常驻：不做 Live2D，不做 VRM，不做角色渲染，也不额外实现一套长期记忆数据库。

### 主要功能

- macOS 原生置顶胶囊浮窗，使用毛玻璃风格面板。
- 需要时展开聊天记录，失焦后自动收回。
- 通过 JSON 配置多个 agent，不写死私人路径和名字。
...
FloatScope 的第一入口是图形界面，不需要记控制指令。
...
定时读屏使用设置页里的默认间隔。除非手动开启 Screen Watch 或 Screen Replay Cache，否则 FloatScope 不会持续截屏。
...
从状态栏气泡图标打开 Settings，配置每个 agent 的路径后再发送消息。公开版默认不会内置任何本机可执行文件路径。
...
设置页可以继续添加更多 agent。暂时没有专门桥接的工具，可以先用 `generic-cli` 接入。
...
第一次截图可能会触发 macOS 的屏幕录制权限提示。授权后，如果系统没有立刻放行，重启 FloatScope 即可。

### 隐私模型

- 公开版默认不包含私人名字、私人路径或本机可执行文件路径。
- FloatScope 只保存轻量 UI 状态、设置、最近本地转写和临时截图文件。
- 长期记忆和完整聊天历史交给配置好的 agent 应用或 CLI。
- 默认不持续截屏，只有手动截图或显式开启相关功能时才会读取屏幕。
...
`.build/` 里的构建产物不会提交。用户本地转写、截图、运行时 agent 配置也会被 git 忽略。
